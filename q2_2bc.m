...
if 0
    ...
    bestf = 2.7; bestb = 15; %For OnePlus 5 Camera
    f = bestf;
    b = bestb;
...
if vis
    ...
    imshow(I1);
    hold on
    scatter(mp1(:,1),mp1(:,2),'marker','o','MarkerFaceColor','r');    
    scatter(EPleft(:,1),EPleft(:,2),'marker','sq','MarkerFaceColor','y');    
    for i = 1:size(M1,1)
    ...
    imshow(I2);
    hold on
    scatter(mp2(:,1),mp2(:,2),'marker','o','MarkerFaceColor','r');    
    scatter(EPright(:,1),EPright(:,2),'marker','sq','MarkerFaceColor','y');    
    for i = 1:size(M2,1)
    ...
    suptitle('Epipolar Lines and Epipoles for Inliers');  
    hold off        
end
if vis2
    % Disparity Map
